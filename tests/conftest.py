...
from yolken_test_8b import YolkenTest8b, AsyncYolkenTest8b, DefaultAioHttpClient
from yolken_test_8b._utils import is_dict
...
logging.getLogger("yolken_test_8b").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[YolkenTest8b]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with YolkenTest8b(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncYolkenTest8b]:
